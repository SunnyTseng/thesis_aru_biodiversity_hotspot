...
###
### Get the validation dataset ready (set threshold as 0.85)
###
...
###
### Data preparation for ARU bird list
### 
...
data_validated <- read_csv(here("data", "JPRF_species_list",  "species_list_above_85_validation.csv")) %>%
  drop_na(best) %>%
  filter(!common_name %in% c("Pygmy Nuthatch", "Red-naped Sapsucker", "Williamson's Sapsucker", "Bay-breasted Warbler", "Bushtit", "Northwestern Crow", "Slate-colored Fox Sparrow", "Audubon's Warbler"))
...
data_validated_1 <- left_join(data_validated, data_species) %>%
  mutate(common_name = case_when(
    ...
    common_name == "Audubon's Warbler" ~ "Yellow-rumped Warbler (Audubon's)",
    common_name == "Northwestern Crow" ~ "American Crow",
    common_name == "Slate-colored Fox Sparrow" ~ "Fox Sparrow (Slate-colored)",
    TRUE ~ common_name)) %>%
  left_join(clements_species, by = c("common_name" = "English name")) %>%
  select(common_name, "scientific name", "order", "family", 14:41)


# write_csv(data_validated_1, here("data", "JPRF_species_list", "species_list_above_85_validation_info.csv"))



###
### Recording validation for the 0.85 confidence (126 species for validation)
###
...
###
### Data preparation for eBird bird list
###
